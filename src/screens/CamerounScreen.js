...
export default function CamerounScreen() {
  ...
  function openModal(type) {
    setModal(type === 'ravit'
      ? { title: 'Ravitaillement', isPlus: true }
      : { title: 'Transfert envoyé', isPlus: false });
  }
  ...
  return (
    <View style={s.container}>
      <ScrollView>
        <SoldeCard
          label="Solde Compte Cameroun"
          solde={solde}
          icon="location-outline"
          iconColor={COLORS.info}
          leftLabel="Ravitaillements" leftVal={'+' + fmtMontant(totalIn)} leftColor={COLORS.info}
          rightLabel="Transferts" rightVal={'-' + fmtMontant(totalOut)} rightColor={COLORS.danger}
        />
        <FilterBar active={filter} onChange={setFilter} />
        <View style={s.actions}>
          <TouchableOpacity style={[s.btn, { borderColor: COLORS.info + '60', backgroundColor: COLORS.infoBg }]} onPress={() => openModal('ravit')}>
            <Ionicons name="cash" size={20} color={COLORS.info} />
            <Text style={[s.btnText, { color: COLORS.info }]}>Ravitaillement</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[s.btn, { borderColor: COLORS.warning + '60', backgroundColor: COLORS.warningBg }]} onPress={() => openModal('transfert')}>
            <Ionicons name="send" size={18} color={COLORS.warning} />
            <Text style={[s.btnText, { color: COLORS.warning }]}>Transfert envoyé</Text>
          </TouchableOpacity>
        </View>
